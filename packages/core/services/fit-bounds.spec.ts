...
describe('FitBoundsService', () => {
  ...
  beforeEach(fakeAsync(() => {
    ...
    TestBed.configureTestingModule(
        {providers: [{provide: MapsAPILoader, useValue: loader}, FitBoundsService]});
  ...
  it('should emit empty bounds when API finished loaded but the are not entries in the includeInBounds$ map',
     fakeAsync(() => {
  ...
  it('should provide all latLng to the bounds', fakeAsync(() => {
       ...
       fitBoundsService.getBounds$().subscribe(success);
       tick(1);
       const latLngs = [{lat: 2, lng: 2}, {lat: 3, lng: 3}, {lat: 4, lng: 4}];
  ...
  it('should remove latlng from bounds and emit the new bounds after the sample time',
     fakeAsync(() => {
       const success = jest.fn();
       fitBoundsService.getBounds$().subscribe(success);
       tick(1);
